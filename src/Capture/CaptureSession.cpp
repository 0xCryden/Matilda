...
CaptureSession::CaptureSession(
    ...
    : m_packetStore(packetStore)
    , m_notifier(notifier)
    , m_connTracker(connTracker)
    , m_device(nullptr)
    , m_running(false)
    , m_callback(nullptr)
    , m_uiWindow(nullptr)
...
bool CaptureSession::start(PacketCallback cb, int deviceIndex, void* uiWindow)
{
    if (m_running)
        return false;
    ...
    m_thread = std::thread([this, deviceIndex]()
        {
            ...
            m_device = std::make_unique<CaptureDevice>(dev);
            ...
            m_device->startCapture(
                [this, localIp](pcpp::RawPacket* rawPacket, pcpp::PcapLiveDevice*, void*)
                {
                    ...
                    // Feed raw bytes to the connection tracker
                    ...
                    DWORD pid = 0;
                    if (proto == "TCP")
                        pid = ProcessMapper::getPidForTcp(srcIp, dstIp, srcPort, dstPort);
                    else if (proto == "UDP")
                        pid = ProcessMapper::getPidForUdp(srcIp, dstIp, srcPort, dstPort);
                    if (pid != 0)
                        procName = ProcessMapper::getProcessNameByPid(pid);
                    ...
                    // Enqueue notification; if not flushed immediately, call callback with empty string
...
bool CaptureSession::isRunning() const
{
    return m_running;
}

std::string CaptureSession::getLocalIpAddress() const
{
    if (!m_device) return "";
    auto dev = m_device->getDevice();
    if (!dev) return "";
    try {
        return dev->getIPv4Address().toString();
    }
    catch (...) {
        return "";
    }
...
CaptureDevice* CaptureSession::getCurrentDevice() const
{
    return m_device.get();
}
